...
import { getTagList, billAdd, updateBillInfo } from '@/api/bill'
...
const PopupAddBill = forwardRef(({ detail = {}, onReload }, ref) => {
  ...
  useEffect(async () => {
    ...
    const _expense = data.filter(item => item.type === '1') // 支出类型
    const _income = data.filter(item => item.type === '2') // 收入类型
    setExpense(_expense)
    setIncome(_income)
    // 没有 id 的情况下，说明是新建账单。
    if (!id) {
  ...
  const addBill = async () => {
    ...
    try {
      if (id) {
        params.id = id
        // 如果有 id 需要调用详情更新接口
        await updateBillInfo(params)
        Toast.show('修改成功')
      } else {
        const result = await billAdd(params)
        setAmount('')
        setPayType('expense')
        setCurrentType(expense[0])
        setDate(new Date())
  ...
  >
    <div className={s.addWrap}>
      ...
      <div className={s.filter}>
        ...
        <div className={s.time} onClick={handleDatePop}>{dayjs(date).format('MM-DD')} <Icon className={s.arrow}
                                                                                            type="arrow-bottom"/></div>
      </div>
      ...
      <div className={s.typeWarp}>
        <div className={s.typeBody}>
          {
            (payType == 'expense' ? expense : income).map(item => <div onClick={() => choseType(item)} key={item.id}
