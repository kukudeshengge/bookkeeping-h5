...
import { getMonthStatistics } from '@/api/bill'
...
const Data = () => {
  ...
  const getData = async () => {
    setLoading(true)
    const { data } = await getMonthStatistics({ date: currentMonth })
    // 总收支
    setTotalExpense(data.total_expense)
    setTotalIncome(data.total_income)
    
    // 过滤支出和收入
    const expense_data = data.list.filter(item => item.pay_type == 1).sort((a, b) => b.number - a.number) // 过滤出账单类型为支出的项
    const income_data = data.list.filter(item => item.pay_type == 2).sort((a, b) => b.number - a.number) // 过滤出账单类型为收入的项
    setExpenseData(expense_data)
    setIncomeData(income_data)
    setLoading(false)
    // 绘制饼图
    const isExpense = pieType === 'expense'
    setPieChart(isExpense ? expense_data : income_data, isExpense ? '支出' : '收入')
  }
  ...
  const changePieType = (type) => {
    ...
    setPieChart(isExpense ? expenseData : incomeData, isExpense ? '支出' : '收入')
  }
  
  // 绘制饼图方法
  const setPieChart = (data, name) => {
    if (echarts) {
      const el = document.getElementById('proportion')
      if (!el) return
      proportionChart = echarts.init(el)
  ...
  const processList = totalType == 'expense' ? expenseData : incomeData
  return <div className={s.data}>
    ...
    <div className={s.structure}>
      ...
      <div className={s.content}>
        {
          !processList.length && !loading ? <div className={cx({ [s.empty]: true })}>
            <img src={img} alt=""/>
          </div> : null
        }
        {
          processList.map(item => <div key={item.type_id} className={s.item}>
